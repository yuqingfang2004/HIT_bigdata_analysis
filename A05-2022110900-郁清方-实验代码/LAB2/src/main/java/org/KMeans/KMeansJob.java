...
public class KMeansJob {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length < 5) {
            System.err.println("输入: KMeansJob <输入> <输出> <分类> <最大迭代次数> <收敛值>");
            System.exit(-1);
        }
        ...
        int maxIterations = Integer.parseInt(args[3]); // 指定最大
        double convergeIndex = Double.parseDouble(args[4]); // 收敛值，即聚类中心的坐标变化均小于这个值，认为收敛——Yu
        ...
        while (iteration < maxIterations && !converged) {
            ...
            String newCentroidsPath = "hdfs:///LAB2/KMeansCentroids" + "/centroids" + iteration;
        ...
        System.out.println("========================================");
        System.out.println("K-Means 在 " + iteration + " 次迭代时收敛/已经达到最大迭代次数");
        System.out.println("========================================");


        // 收敛后或达到最大收敛次数时进行数据聚类并输出结果
        Job finalJob = Job.getInstance(conf, "Final Clustering");

        finalJob.setJarByClass(KMeansJob.class);
        finalJob.setMapperClass(FinalKMeansMapper.class);
        finalJob.setReducerClass(FinalKMeansReducer.class);
        ...
        finalJob.addCacheFile(new URI(centroidsPath));//结束迭代时候的最终聚类中心文件路径，在Reducer读取，进行分类——Yu
    ...
    private static String generateInitialCentroids(Configuration conf, String inputPath, int K) throws Exception {
        ...
        Path centroidsPath = new Path("hdfs:///LAB2/KMeansCentroids/initial_centroids.txt");
